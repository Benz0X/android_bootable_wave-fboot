...
//DISPLAYMENU
        ...
        {
            strcpy(title,"  Booting kernel 1 in Recov  ");
            kernelImage = L"/e/boot.img";
            bootMode=1;
        }
        if(mode == rm_FOTA_SECOND)
        {
            strcpy(title,"  Booting kernel 2 in Normal  ");
            kernelImage = L"/e/boot_2.img";
        }
        if(mode == rm_FOTA_RECOSECOND)
        {
            strcpy(title,"  Booting kernel 2 in Recov   ");
            kernelImage = L"/e/boot_2.img";
            bootMode=1;
        }
//Tests
        ...
        strcpy(entry1,"        Press any key         ");
        strcpy(entry2,"              to              ");
        strcpy(entry3,"           interrupt          ");
        drawEntry(currentMenu,entry1,entry2,entry3,title);

        while(pass==0&&count<1000000)
        {
            if (is_key_pressed(1,1)||is_key_pressed(1,2)||is_key_pressed(0,0)||is_key_pressed(2,1)||is_key_pressed(2,0))
            {
                pass=1;
            }
            count++;
